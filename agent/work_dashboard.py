...
<body>
  ...
  <script>
    ...
    const requestedWorker = new URLSearchParams(window.location.search).get('worker') || '';
    ...
    function renderPerson(person) {
      ...
      const correction = Array.isArray(person.available_tasks) && person.available_tasks.length
        ? `<details class="day-card" ${state.intern === person.user_key && !person.active_task_id ? 'open' : ''}><summary>Assign or correct active ClickUp task/project</summary><p class="muted">Choose an existing task when it fits. The correction is audited and applies from now forward; it does not silently rewrite prior paid time.</p><div class="key-grid"><select data-task-correction-task="${escapeHtml(person.user_key)}">${person.available_tasks.map((task) => `<option value="${escapeHtml(task.id)}" ${task.id === person.active_task_id ? 'selected' : ''}>${escapeHtml(task.name)} | ${escapeHtml(task.list_name || 'unknown list')}</option>`).join('')}</select><input data-task-correction-by="${escapeHtml(person.user_key)}" placeholder="Corrected by"><input data-task-correction-reason="${escapeHtml(person.user_key)}" placeholder="Reason for correction"><button type="button" data-task-correction-apply="${escapeHtml(person.user_key)}" data-session-date="${escapeHtml(person.current_session_date)}">Apply assignment</button></div><p data-task-correction-result="${escapeHtml(person.user_key)}"></p></details>`
        : '';
