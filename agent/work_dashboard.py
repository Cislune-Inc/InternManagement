...
async def _build_person_payload(
    ...
    return {
        ...
        "regular_workdays": list(user.regular_workdays),
        "typical_start_time": user.typical_start_time,
        "typical_end_time": user.typical_end_time,
        "planned_time_off": list(user.planned_time_off),
...
<body>
  ...
  <script>
    ...
    const requestedWorker = requestedParams.get('worker') || '';
    const requestedSession = requestedParams.get('session') || '';
    const state = { intern: requestedWorker, query: '' };
