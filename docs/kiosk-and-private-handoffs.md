...
1. At the Mini, choose your name and type your personal 2–6 digit PIN using the keyboard.
2. Press Enter or select Start / return to work. Lunch, paid rest and clock-out are also available.
...
One-time setup: select **Set up / reset PIN** in Don Pollo Project Updates Home,
or the worker sends `kiosk setup` from their own authenticated Slack
...
Four or more digits are recommended: two-digit PINs are easier to guess. Repeated
or simple sequential digits are rejected during setup. Existing six-digit PINs
and hashes continue to work without a reset or data migration. No onscreen keypad.
